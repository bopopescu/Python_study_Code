  ...
  for z in get_host():
    ...
    for username in mysql_username:
      for password in ip_x:
        try:
          db = MySQLdb.connect(host=z, port="3306",user=username, passwd=password,db='met',charset="utf8")
          success = True
          if success:
            print ("%s: %s" % ( threadName, time.ctime(time.time()),username, password ))
